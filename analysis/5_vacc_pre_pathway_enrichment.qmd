...
```{r Bubble plot}
pathway_plot <- 
  c("GOBP_NEGATIVE_REGULATION_OF_VIRAL_PROCESS",
...
result %>% 
  dplyr::select(gs_name:contrast, pathway_enrichment) %>% unnest() %>% 
  filter(pathway %in% pathway_plot) %>% 
  mutate(`-log10 P-value` = -log10(padj)) %>% 
  ggplot(aes(x = contrast, y = pathway, color = NES, size = `-log10 P-value`)) +
  geom_point() +
  facet_grid(~vacc) +
